...
const MINIMAL_SENSITIVITY = 33;
const URL = "http://192.168.43.109:8080/";
const TOP = "U"; //U
const BOTTOM = "D";
const RIGHT = "R";
const LEFT = "L";
const TOP_RIGHT = "UR";
const TOP_LEFT = "UL";
const BOTTOM_RIGHT = "DR";
const BOTTOM_LEFT = "DL";
...
const getDirection = () => {
  const isFar =
    Math.abs(positions.fixedY - positions.innerY) > MINIMAL_SENSITIVITY ||
    Math.abs(positions.innerX - positions.fixedX) > MINIMAL_SENSITIVITY;
  if (!isFar) return;

  let T_CONDITION = angle >= pi / 8 && angle <= (7 * pi) / 8;
  let TR_CONDITION = angle <= pi / 3;
  let TL_CONDITION = angle >= (2 * pi) / 3;
  let B_CONDITION = angle <= -pi / 8 && angle >= -(7 * pi) / 8;
  let BR_CONDITION = angle >= -pi / 3;
  let BL_CONDITION = angle <= -(2 * pi) / 3;
  let R_CONDITION = Math.cos(angle) >= 0;
  let L_CONDITION = Math.cos(angle) < 0;

  if (T_CONDITION)
    return TOP + (TR_CONDITION ? RIGHT : TL_CONDITION ? LEFT : "");

  if (B_CONDITION)
    return BOTTOM + (BR_CONDITION ? RIGHT : BL_CONDITION ? LEFT : "");

  if (R_CONDITION) return RIGHT;

  if (L_CONDITION) return LEFT;
...
function touchMove(x, y) {
  ...
  angle = Math.atan2(
    positions.fixedY - positions.innerY, // tqt
    positions.innerX - positions.fixedX
  );

  // angle >= 2.5 || angle <= -2.5 ? (angle = 3.14) : "";

  // If inner circle is outside joystick radius, reduce it to the circumference
  if (!((x - positions.fixedX) ** 2 + (positions.fixedY - y) ** 2 < 10000)) {
...
async function fetchDirection() {
  ...
  while (conditionLoop) {
    ...
    conditionLoop = direction;

    if (lastSent !== direction) {
      if (
        ARRAY_OF_RESPONSE.filter((value) => {
          return value !== INIT;
        }).includes(res)
      ) {
        console.log("INIT");
...
function renderLoop() {
  ...
  if (speed.fixedX && speed.fixedY) {
    ctx.beginPath();
    ctx.fillStyle = "#0004";
    ctx.arc(speed.fixedX, speed.fixedY, 100, 0, 2 * pi);
    ctx.fill();
    ctx.closePath();

    ctx.beginPath();
    ctx.fillStyle = "#0009";
    ctx.arc(speed.fixedX, speed.fixedY, 70, 0, 2 * pi);
    ctx.fill();
    ctx.closePath();
  } else {
    ctx.beginPath();
  ...
  if (!(positions.fixedX || positions.fixedY)) {
    ...
    ctx.beginPath();
    ctx.fillStyle = "#0004";
    ctx.arc(window.innerWidth / 4, window.innerHeight / 2, 50, 0, 2 * pi);
    ctx.fill();
    ctx.closePath();
    return;
  }
  ...
  ctx.fillText(`Direction: ${getDirection()}`, 20, 140);
